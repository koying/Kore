...
public class PlaybackOverlayFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSelectedMedia = (Media) getActivity()
                .getIntent().getParcelableExtra(LeanbackActivity.MEDIA);

        mItems.add(mSelectedMedia);
        mCurrentItem = 0;
        /*
        HashMap<String, List<Media>> medias = VideoProvider.getMovieList();
        if(medias != null) {
            for (Map.Entry<String, List<Media>> entry : medias.entrySet()) {
                if (mSelectedMedia.getCategory().contains(entry.getKey())) {
                    List<Media> list = entry.getValue();
                    if(list != null && !list.isEmpty()) {
                        for (int j = 0; j < list.size(); j++) {
                            mItems.add(list.get(j));
                            if (mSelectedMedia.getTitle().contentEquals(list.get(j).getTitle())) {
                                mCurrentItem = j;
                            }
                        }
                    }
    ...
    private int getDuration() {
        Media media = mItems.get(mCurrentItem);
        MediaMetadataRetriever mmr = new MediaMetadataRetriever();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            mmr.setDataSource(media.getVideoUrl(), new HashMap<String, String>());
        } else {
            mmr.setDataSource(media.getVideoUrl());
        }
    ...
    private void updatePlaybackRow(int index) {
        if (mPlaybackControlsRow.getItem() != null) {
            Media item = (Media) mPlaybackControlsRow.getItem();
    ...
    public interface OnPlayPauseClickedListener {
        public void onFragmentPlayPause(Media media, int position, Boolean playPause);

        public void onFragmentFfwRwd(Media media, int position);
    }

    static class DescriptionPresenter extends AbstractDetailsDescriptionPresenter {
        @Override
        protected void onBindDescription(ViewHolder viewHolder, Object item) {
            viewHolder.getTitle().setText(((Media) item).getTitle());
            viewHolder.getSubtitle().setText(((Media) item).getStudio());
        }
    }
    ...
    private final class ItemViewClickedListener implements OnItemViewClickedListener {
        ...
        public void onItemClicked(Presenter.ViewHolder itemViewHolder, Object item,
            ...
            if (item instanceof Media) {
                ...
                intent.putExtra(LeanbackActivity.MEDIA, media);

                Bundle bundle = ActivityOptionsCompat.makeSceneTransitionAnimation(
                        getActivity(),
                        ((ImageCardView) itemViewHolder.view).getMainImageView(),
                        LeanbackActivity.SHARED_ELEMENT_NAME).toBundle();
